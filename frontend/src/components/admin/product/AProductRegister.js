...
const AProductRegister = () => {
    ...
    useEffect(() => {
        axios
            ...
            .then((res) => {
                ...
                const uniqueKor = [
                    ...new Map(res.data.product.map((item) => [item.product_id, item.product_name_kor])).values(),
                ];
                const uniqueEng = [
                    ...new Map(res.data.product.map((item) => [item.product_id, item.product_name_eng])).values(),
                ];
    ...
    const submitGo = (me) => {
        ...
        if (korName.includes(data.product_name_kor)) {
            alert('제품 국문명이 이미 등록되어 있습니다. 제품명을 확인해주세요.');
            return;
        }
        ...
        if (engName.includes(data.product_name_eng)) {
            alert('제품 영문명이 이미 등록되어 있습니다. 제품명을 확인해주세요.');
            return;
        }
        ...
        axios
            ...
            .then((res) => {
                console.log('제품 등록 완료했습니다. 해당 제품의 옵션을 등록해주세요.', res.data.newId);
                console.log(res.data.newId);

                alert('제품 등록 완료했습니다. 해당 제품의 옵션을 등록해주세요.');
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="product_category_id" id="product_category_id">
                                    ...
                                    <option value="2">홈 프레그런스 &gt; 캔들</option>
                                    <option value="3">홈 프레그런스 &gt; 디퓨저</option>
                                    <option value="4">배스 앤 바디 &gt; 배스 앤 샤워 &gt; 바디 앤 핸드 워시</option>
                                    <option value="5">배스 앤 바디 &gt; 배스 앤 샤워 &gt; 샤워 젤 앤 오일</option>
                                    <option value="6">배스 앤 바디 &gt; 배스 앤 샤워 &gt; 배스 오일</option>
                                    <option value="7">배스 앤 바디 &gt; 바디 케어 &gt; 바디크림</option>
                                    <option value="8">배스 앤 바디 &gt; 바디 케어 &gt; 바디 앤 핸드 로션</option>
                                    <option value="9">배스 앤 바디 &gt; 바디 케어 &gt; 핸드크림</option>
                                    <option value="10">배스 앤 바디 &gt; 바디 케어 &gt; 바디 미스트</option>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <select name="product_scent" id="product_scent">
                                    ...
                                    <option value="light-floral">라이트 플로랄</option>
