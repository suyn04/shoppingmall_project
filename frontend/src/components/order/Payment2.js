...
function Payment2(props) {
    ...
    function dataInit() {
        // 해당 고객의 장바구니 접근
        axios
            .get(`http://localhost:5001/payment2/${email}`)
            .then(res => {
    ...
    const handleQuantityChange = (id, quantity) => {
        const updatedProd = prod.map(item => (item.bs_product_id === id ? { ...item, quantity: parseInt(quantity, 10) } : item));
        setProd(updatedProd);
    };
    ...
    function orderFin(e) {
        ...
        axios
            .post(`http://localhost:5001/payment2/join/${email}`, orderPayload)
            .then(res => {
                alert('결제되었습니다.');
                // navigate('/payment3')
                setIsLoading(true);
        ...
        axios
            .delete(`http://localhost:5001/payment2/delete/${email}`)
            .then(res => {})
            .catch(err => {
                console.log('삭제오류 : ', err);
            });
